    ...
    public sealed class ContactListener2D : ContactListener
    {
        ...
        private void OnTriggerEnter2D(Collider2D contactCollider2D)
        {
            if (contactType == ContactTypes.Trigger)
                ContactStatus(ContactStatusTypes.Enter, contactCollider2D.gameObject.tag, null, null, null,
                    contactCollider2D);
        }
        private void OnTriggerStay2D(Collider2D contactCollider2D)
        {
            if (contactType == ContactTypes.Trigger)
                ContactStatus(ContactStatusTypes.Stay, contactCollider2D.gameObject.tag, null, null, null,
                    contactCollider2D);
        }
        private void OnTriggerExit2D(Collider2D contactCollider2D)
        {
            if (contactType == ContactTypes.Trigger)
                ContactStatus(ContactStatusTypes.Exit, contactCollider2D.gameObject.tag, null, null, null,
                    contactCollider2D);
        }
        #endregion
